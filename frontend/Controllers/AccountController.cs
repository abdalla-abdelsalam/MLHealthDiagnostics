    ...
    public class AccountController : Controller
    {
      ...
      //  [ValidateAntiForgeryToken]
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            ApplicationDbContext mode = new ApplicationDbContext();
            bool u = mode.Users.Where(e => e.UserName == model.UserName && e.EmailConfirmed == true).Any();
            if (u)
            {
                // This doesn't count login failures towards account lockout
                // To enable password failures to trigger account lockout, change to shouldLockout: true
                var result = await SignInManager.PasswordSignInAsync(model.UserName, model.Passward, model.RememberMe, shouldLockout: false);
       ...
       // [ValidateAntiForgeryToken]
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(user);//add in DB
                if (result.Succeeded)
                {
                    result = await UserManager.AddLoginAsync(user.Id, info.Login);//make login
                    if (result.Succeeded)
                    {

                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);//make cookes
                        UserManager.AddToRole(user.Id, "User");
                        return RedirectToLocal(returnUrl);
                    }
                }
